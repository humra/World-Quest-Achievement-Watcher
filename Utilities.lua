...
function WQA:GetTaskTime(task)
    ...
    elseif task.type == "AREA_POI" then
        ...
        if schedulerInfo and schedulerInfo.endTime then
            ...
            return math.max(0, schedulerInfo.endTime - now) / 60
        end
        -- Scenario-backed public events do not expose an Area POI timer. Do
        -- not query the synthetic scenario ID through C_AreaPoiInfo.
        if self:GetScenarioAreaPoiInfo(task.id, task.mapId) then
            return nil
        end
...
function WQA:GetTaskLink(task)
    ...
    elseif task.type == "AREA_POI" then
        ...
        local entry = self.Criterias.AreaPoi.list[task.id] and self.Criterias.AreaPoi.list[task.id][task.mapId]
        return (entry and entry.name) or "World Event"
    end
end
